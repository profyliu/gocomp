...
$onecho > solution1.txt
This is dummy contents for testing.
--bus section
i, v(p.u.), theta(deg), bcs(MVAR at v = 1 p.u.)
1           ,        1.100000000000000,      -46.064922787932450,        0.000000000000000
2           ,        1.096798177829484,      -46.695637095542410,        0.000000000000000
3           ,        1.069796048798548,      -51.375898758686730,        0.000000000000000
4           ,        1.073271422416586,      -48.842151799279340,        0.000000000000000
5           ,        1.076797235039777,      -47.576986986850700,        0.000000000000000
6           ,        1.100000000000000,      -43.466789412463780,        0.000000000000000
7           ,        1.090412444498625,      -49.232031159732860,        0.000000000000000
8           ,        1.100000000000000,      -49.205119579245900,        0.000000000000000
9           ,        1.080763047857875,      -49.457396804502700,        0.000000000000000
10          ,        1.075396834113930,      -48.681921896134280,        0.000000000000000
11          ,        1.082323260408401,      -46.253748062611690,        0.000000000000000
12          ,        1.086670866090186,      -44.554114547713540,        0.000000000000000
13          ,        1.080336780434941,      -44.991105756363600,        0.000000000000000
14          ,        1.063882132548582,      -48.340645138058240,        0.000000000000000
--generator section
i, id, p(MW), q(MVAR)
1           ,1 ,       37.964960679191880,        0.454205569312485
2           ,1 ,       84.645212132308780,       22.191099474505220
3           ,1 ,        5.801788265816882,       26.699836480062030
6           ,1 ,      110.503275580704200,      -15.273183452983570
8           ,1 ,        0.319828670471907,        5.987194640744570
$offecho


scalar
    all_start
    avail_time /%timelimit%/
;
all_start = timeelapsed;
...
$ontext
solve ext_base_only minimizing v_c using nlp;

* Fix base case solution
v_vi.fx(i) = v_vi.l(i);
v_pg.fx(i,g) = v_pg.l(i,g);
v_qg.fx(i,g) = v_qg.l(i,g);
v_bics.fx(i) = v_bics.l(i);
v_cg.fx(i,g) = v_cg.l(i,g);
v_csigma.fx = v_csigma.l;
v_ped.fx(i,i,ckt) = v_ped.l(i,i,ckt);
v_peo.fx(i,i,ckt) = v_peo.l(i,i,ckt);
v_pfd.fx(i,i,ckt) = v_pfd.l(i,i,ckt);
v_pfo.fx(i,i,ckt) = v_pfo.l(i,i,ckt);
v_qed.fx(i,i,ckt) = v_qed.l(i,i,ckt);
v_qeo.fx(i,i,ckt) = v_qeo.l(i,i,ckt);
v_qfd.fx(i,i,ckt) = v_qfd.l(i,i,ckt);
v_qfo.fx(i,i,ckt) = v_qfo.l(i,i,ckt);
v_tgh.fx(i,g,h) = v_tgh.l(i,g,h);
v_thetai.fx(i) = v_thetai.l(i);
v_sigmaenS.fx(i,i,ckt,n) = v_sigmaenS.l(i,i,ckt,n);
v_sigmainPp.fx(i,n) = v_sigmainPp.l(i,n);
v_sigmainPn.fx(i,n) = v_sigmainPn.l(i,n);
v_sigmainQp.fx(i,n) = v_sigmainQp.l(i,n);
v_sigmainQn.fx(i,n) = v_sigmainQn.l(i,n);
v_sigmafnS.fx(i,i,ckt,n) = v_sigmafnS.l(i,i,ckt,n);
v_sigmaiPp.fx(i) = v_sigmaiPp.l(i);
v_sigmaiPn.fx(i) = v_sigmaiPn.l(i);
v_sigmaiQp.fx(i) = v_sigmaiQp.l(i);
v_sigmaiQn.fx(i) = v_sigmaiQn.l(i);
v_sigmaeS.fx(i,i,ckt) = v_sigmaeS.l(i,i,ckt);
v_sigmafS.fx(from,to,ckt) = v_sigmafS.l(from,to,ckt);
$offtext
...
set gen_conts(k)
    ...
    xfmr_conts(k)
;
gen_conts(k) = yes$(sum((i,g)$GeneratorOutofServiceEvent(i,g,k), 1) > 0);
$offtext
...
lineq_17(from,to,ckt,k)$(actline(from,to,ckt) and activek(k))..
    p_sigmaekS_this(from,to,ckt) =e= sum(n, v_sigmaeknS(from,to,ckt,k,n));

lineq_19(from,to,ckt,k)$(actxfmr(from,to,ckt) and activek(k))..
    p_sigmafkS_this(from,to,ckt) =e= sum(n, v_sigmafknS(from,to,ckt,k,n));

lin_obj_current..
    lin_c_S =e= sum(k$activek(k), sum(n, p_lambdanS(n)*sum((from,to,ckt)$actlinek(from,to,ckt,k), v_sigmaeknS(from,to,ckt,k,n))
                       + p_lambdanS(n)*sum((from,to,ckt)$actxfmrk(from,to,ckt,k), v_sigmafknS(from,to,ckt,k,n))
                ));
...
lin_S_model.solvelink = 5;


$ontext
positive variable
    benders_v_sub_cost(k) dummy for the subproblem cost in the master objective
;
variables
    benders_v_c overall objective value
;
equation
    benders_master_objfun
;

benders_master_objfun..
    benders_v_c =e= sum((i,g)$actgen(i,g), v_cg(i,g)) + p_delta*v_csigma + (1-p_delta)/card(k)*sum(k,benders_v_sub_cost(k));


model benders_master /
    benders_master_objfun
    eq_2
    eq_3
    eq_5
...
p_vi(i) = v_vi.l(i);
p_qg(i,g) = v_qg.l(i,g);
p_thetai(i) = v_thetai.l(i);
p_bics(i) = v_bics.l(i);



loop(k0,
    affected_MW(k0) = sum((i,g)$(actgen(i,g) and GeneratorOutofServiceEvent(i,g,k0)), v_pg.l(i,g))
                      + sum((i,j,ckt)$(actline(i,j,ckt) and BranchOutofServiceEvent(i,j,ckt,k0)), abs(v_peo.l(i,j,ckt)) + abs(v_qeo.l(i,j,ckt)));
);
display affected_MW;
...
set affected_selected_cont(k);
affected_selected_cont(k) = no;
scalar selected_cont_ord, selected_cont_val;
parameter selected_cont_priority(k);
selected_cont_priority(k) = 0;

loop(select_iter$(ord(select_iter) <= card(k)),
    selected_cont_val = smax(k$(not affected_selected_cont(k)), affected_MW(k));
    selected_cont_ord = smin(k$(not affected_selected_cont(k) and affected_MW(k) >= selected_cont_val), ord(k));
    affected_selected_cont(k)$(ord(k) = selected_cont_ord) = yes;
    selected_cont_priority(k)$(ord(k) = selected_cont_ord) = ord(select_iter);
);
display affected_selected_cont, selected_cont_priority;


parameter
    move_on /0/
    how_many_cont_looked /0/
    time_remaining
    sol_fix_ck(k)
;

* Handle generator contintencies
loop(select_iter$(move_on = 0),
loop(k0$(affected_selected_cont(k0) and selected_cont_priority(k0) eq ord(select_iter)),
...
$ontext
    ...
    cont_cputime_nlp(k0) = timeelapsed - starttime;
    cont_mod_stat_nlp(k0) = ext_single_cont.modelstat;
    cont_v_c(k0) = v_c.l;
$offtext
...
$ifthen %final_solve% == rminlp
    v_xgkPn.fx(i,g,k0) = v_xgkPn.l(i,g,k0);
    v_xgkPp.fx(i,g,k0) = v_xgkPp.l(i,g,k0);
    v_xgkQn.fx(i,g,k0) = v_xgkQn.l(i,g,k0);
    v_xgkQp.fx(i,g,k0) = v_xgkQp.l(i,g,k0);
$endif
    ...
    how_many_cont_looked = how_many_cont_looked + 1;

    time_remaining = avail_time - (timeelapsed - all_start);
    if(time_remaining < proportion_time_for_final_nlp*avail_time,
        move_on = 1;
    );
);
);

display move_on, how_many_cont_looked;
display sol_fix_ck;
...
parameter base_case_cost;
base_case_cost = sum((i,g)$actgen(i,g), v_cg.l(i,g)) + p_delta*v_csigma.l;
display base_v_c, base_case_cost, v_c.l;

$ontext
option nlp=pathnlp;
ext_base_only.iterlim=0;
solve ext_base_only minimizing v_c using nlp;
$offtext

sol_v_vi(i) = v_vi.l(i);
sol_v_thetai(i) = v_thetai.l(i)*(180/pi);
sol_v_biCS(i) = p_s_tilde * v_biCS.l(i);
sol_v_pg(i,g) = p_s_tilde * v_pg.l(i,g);
sol_v_qg(i,g) = p_s_tilde * v_qg.l(i,g);


execute_unload 'solution1.gdx' sol_v_vi, sol_v_thetai, sol_v_biCS, sol_v_pg, sol_v_qg;
file soltext /'solution1.txt'/;

put soltext;
put '--bus section' /;
put 'i, v(p.u.), theta(deg), bcs(MVAR at v = 1 p.u.)' /;
loop(i,
    put  i.tl  ',' sol_v_vi(i):25:15 ',' sol_v_thetai(i):25:15 ',' sol_v_biCS(i):25:15 /;
);
put '--generator section' /;
put 'i, id, p(MW), q(MVAR)' /;
loop((i,g)$gen(i,g),
    put  i.tl ',' "'" g.tl:2:0 "'" ',' sol_v_pg(i,g):25:15 ',' sol_v_qg(i,g):25:15 /;
);

putclose soltext;

scalar total_time_used;
total_time_used = timeelapsed - all_start;
display total_time_used;
